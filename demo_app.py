...
with st.sidebar:
    ...
    shopper_labels = [
        f"...{str(r['loyalty_card_key'])[-8:]}  ·  {r.get('segment','?')}"
...
with hc1:
    masked_id = f"...{str(shopper['loyalty_card_key'])[-8:]}"
    st.markdown(
...
with st.expander("🔍 Business Intelligence Panel — Jury View", expanded=False):
    ...
    with bj2:
        ...
        if not seg_df.empty:
            ...
            st.metric("Avg visits/month",     f"{seg_df['frequency_per_month'].mean():.1f}")
            pct_elig = seg_df["reward_eligible"].mean() * 100
    ...
    summary = (
        df.groupby("segment")
        .agg(
            Shoppers       = ("loyalty_card_key",           "count"),
